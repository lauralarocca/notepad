...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
    ...
    function handleSaveNote(event: FormEvent) {
        ...
        if (content === "") {
            toast.warning("Não é possível salvar nota em branco!")
            return
        }
    ...
    function handleStartRecording() {
        ...
        if (!isSpeechRecognitionAPIAvailable) {
            toast.warning("Seu navegador não suporta a API de gravação")
            return
        }
                        ...
                        <div className="flex flex-1 flex-col gap-3 p-5">
...

                            {
                                shouldShowOnboarding ?
                                    (
                                        <p className="text-sm leading-6 text-slate-400">
                                            Comece <button type="button" onClick={handleStartRecording} className="font-medium text-lime-400 hover:underline">gravando uma nota</button> em áudio ou se preferir <button className="font-medium text-lime-400  hover:underline" type="button" onClick={handleStartEditor}>utilize apenas texto</button>.
                                        </p>
                                    ) :
                                    (
                                        <textarea
